...
{
    public class ImageGalleryPopupViewModel : BasePopupViewModel
    {
        #region Properties
        ...
        ImageGalleryPopupPage _imageGallery;
        #endregion
        ...
        public ImageGalleryPopupViewModel(ObservableCollection<AttachmentDBO> attachment, ImageGalleryPopupPage imageGalleryPopup)
        {

            if (attachment != null)
            {
                IsDeleteVisible = true;
                LogAttachment = new ObservableCollection<AttachmentDBO>(attachment.Where(x => x.IsMedia).ToList());
            }
            else
                IsDeleteVisible = false;
            _imageGallery = imageGalleryPopup;
        ...
        {
            AttachmentDBO attachment = (AttachmentDBO)_attachment;
            var imagePopup = new ShowImagePopup(attachment);
            await NavigationService.Nav.PushPopupAsync(imagePopup);
        }
        ...
        private async void DeleteImage(object _attachment)
        {
            try
            {
                bool val = await _imageGallery.DisplayAlert("", "Are you sure you want to delete?", "Yes", "No");
                if (val)
                {
                    AttachmentDBO attachment = (AttachmentDBO)_attachment;
